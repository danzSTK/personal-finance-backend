  ...
  EmailMessage.reconstitute(
    {
      ...
      templateParams: {},
      idempotencyKey: PasswordChangeBlockedEmailIdempotencyKeys.event('block-event-1'),
      status,
...
describe('CreatePasswordChangeBlockedEmailMessageUseCase', () => {
  ...
  let useCase: CreatePasswordChangeBlockedEmailMessageUseCase;
  ...
  beforeEach(async () => {
    ...
    useCase = moduleRef.get(CreatePasswordChangeBlockedEmailMessageUseCase);
  });

  describe('execute', () => {
    it('creates a password change blocked intent from the persisted user and event context', async () => {
      ...
      expect(result.emailMessage.type).toBe(EmailMessageType.PASSWORD_CHANGE_BLOCKED);
      expect(result.emailMessage.recipientEmail).toBe('daniel@example.com');
      expect(result.emailMessage.templateKey).toBe(EmailTemplateKey.PASSWORD_CHANGE_BLOCKED);
      expect(result.emailMessage.templateVersion).toBe(EmailTemplateVersion.V1);
      expect(result.emailMessage.idempotencyKey).toBe('email:password-change:event:block-event-1');
    ...
    it('uses safe fallbacks for a missing name and incomplete security context', async () => {
      findByIdempotencyKey.mockResolvedValue(null);
      findUserById.mockResolvedValue(makeUser(null, 'fallback@example.com'));
      saveEmailMessage.mockImplementation(message => Promise.resolve(message));
