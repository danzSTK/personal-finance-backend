...
O resultado esperado é um `MailService` reutilizável por um futuro módulo `notifications`, capaz de delegar o envio para adapters como Brevo SDK/API ou outro provedor transacional.
...
- criação de adapter Brevo SDK/API como primeira implementação;
- desenho de uma porta que permita trocar o adapter no futuro sem mudar consumidores;
...
THE SYSTEM SHALL NOT exigir import de Brevo ou SDK concreto.

### REQ-003 - Usar Brevo SDK/API como adapter inicial

WHEN o provedor configurado for `brevo`
THE SYSTEM SHALL enviar e-mails transacionais por meio do SDK oficial da Brevo.

### REQ-004 - Permitir troca de provedor

WHEN o provedor configurado mudar para outro adapter suportado no futuro
...
- O design deixa claro como trocar Brevo por outro provider via adapter.
