...
- Objetivo: validar as rotas de alteração e status, DTOs, pipes, filtro global,
  cookies e `Retry-After`.
...
- status disponível retorna somente o contrato booleano, sem `Retry-After`;
- status indisponível retorna `200`, booleano falso e `Retry-After`, sem causa.

## Composição e dependências

Usa `AuthController`, ValidationPipe, filtro e Supertest reais. O usuário e o token
são anexados por middleware de teste. Os dois use cases de password change são
mockados e `PasswordChangeCostGuard` é substituído. Não usa Docker, rede ou
credenciais reais.
